...
本次验证：格式检查、`git diff --check`、GUI crate 全部测试（93 通过，3 项设备/外部文件相关测试跳过）及 `cargo clippy --workspace --all-targets --locked -- -D warnings` 通过。新增离线目录选择、来源导出、原格式副本和窗口布局回归测试。Universal Release、DMG 挂载与完整性、签名和远端下载校验通过；已检查本地启动待机画面、记录管理和重新测量菜单。当前未发现 KM003C，真机尚未验证；未找到用户所述退出错误对应的 panic 或 KM003C 崩溃报告。
