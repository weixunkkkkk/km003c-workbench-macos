...
- 记录管理从设置页独立出来，恢复会话和设备离线记录分别管理，按钮列固定对齐。
- 离线目录刷新会保留当前文件名和数据地址选择；下载失败会清除旧离线视图，避免误导出上一条记录。
- 导入文件支持“返回实时”，导出时可另存原格式副本，并保留 `.km003c.json` 元数据，不覆盖原文件。
- 离线记录末点与设备摘要存在有限舍入或末间隔差异时，保留原始采样并给出提示，不虚构补点。
- 设置分类、表单、图表统计和紧凑窗口重新对齐；中英文切换不会把长文案或控件推出窗口。
- 新包 App Build 为 `3`，DMG SHA-256 为 `5505f1f6f8606a952b6c2223316a1c39b32d174f4b545eab28d1d68e74dd077c`。
...
- **可切换皮肤**：在设置 → 通用中选择“工业仪器”或“日系风格”；后者使用内置日系角色壁纸铺满工作台，并以半透明深色面板保证测量数据清晰，录制中换肤不会暂停或清空数据。
- **记录管理**：恢复录制和设备离线记录使用独立管理窗口；刷新目录不会丢失当前选择，导入文件可返回实时数据源或另存原格式副本。
- **后台录制修复**：采样处理与录制写入不再依赖窗口绘制，最小化或隐藏窗口后仍继续处理数据。验证范围和真机未完成项目见[后台录制验收记录](docs/BACKGROUND-RECORDING-FIX.md)。
...
协议解析、测量、录制、导入导出和 GUI 单元测试可以在没有设备时运行。最新日期版 Universal
构建、DMG 完整性、Info.plist 和 ad-hoc 签名已经过本地验证；KM003C 真机上的
USB 锁屏续录、拔插恢复和完整协议协商仍需要连接设备进行验收。

详细的版本变更和验证边界见 [`CHANGELOG.md`](CHANGELOG.md) 与
[`docs/RELEASE-2026-09-19.md`](docs/RELEASE-2026-09-19.md)。
